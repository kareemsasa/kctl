...
The dashboard also includes an Actions panel. From there you can refresh the index immediately, create a plan file from a built-in template, or start `plans run-many` for a plans directory. Run-many launches in a background thread and reindexes the repository when it completes so new runs show up in the UI.
